<x-layout bodyClass="g-sidenav-show  bg-gray-200">
    ...
    <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg ">
        ...
        <div class="p-4">
            <a href="{{ route('search-terms.index') }}"><i class="fas fa-arrow-left"></i></a>
            <form method="POST" action="{{ route('search-terms.update', ['search_term' => $searchTerm->id]) }}">
                @csrf
                @method('PUT')
                <div class="mb-3">
                ...
                <div class="mb-3" id="replies_content">
                    ...
                    @foreach($searchTerm->keyword->replies as $reply)
                        <textarea style="border: solid black 1px; outline: 0" name="replies[{{$reply->id}}]" data-id="{{ $reply->id }}" id="replies_{{$reply->id}}" class="form-control mt-2 replies">{{ $reply->reply }}</textarea>
                    @endforeach
                </div>
